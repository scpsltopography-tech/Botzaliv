...
# 100 — это ещё и жёсткий лимит Telegram Bot API на forward_messages() за один вызов,
# выше в принципе не имеет смысла (да и админу неудобно листать пост из 300 видео).
...
# Необязательная обратная совместимость: если в .env всё ещё указан CHANNEL_ID
# (старый способ) — бот сам зарегистрирует его как канал при старте.
...
async def db_init(dsn: str):
    ...
    async with _pool.acquire() as conn:
        ...
        # каналы, куда бот добавлен админом — определяются автоматически
        await conn.execute(
            """
...
BATCH_STATES = {
    ...
    Batch.waiting_hashtag.state, Batch.waiting_channel.state,
}
...
async def update_counter(bot: Bot, chat_id: int, status_msg_id: Optional[int], text: str,
    ...
    """Редактирует счётчик на месте вместо удаления+пересоздания. Возвращает message_id."""
    if status_msg_id:
        try:
...
async def cmd_help(message: Message):
    ...
    if await is_admin(uid):
        lines += [
            ...
            "/batch — начать приём контента и опубликовать в канал напрямую (только в служебной группе)",
    ...
    lines += [
        ...
        "\"Публикация сообщений\", он сам это обнаружит. Можно добавить в несколько каналов "
        "сразу — при публикации бот спросит, куда именно.",
    ]
...
async def get_hashtag(message: Message, state: FSMContext):
    ...
    await choose_channel_and_publish(message.bot, message.chat.id, message.from_user.id, state, reply_to=message)


async def choose_channel_and_publish(bot: Bot, chat_id: int, actor_id: int, state: FSMContext, reply_to: Optional[Message] = None):
    channels = await list_channels()
...
async def publish_post(bot: Bot, chat_id: int, actor_id: int, state: FSMContext, channel_id: int):
    ...
    await bot.send_message(chat_id, f"✅ Готово! Опубликовано в канал.\nСсылка на контент: {deep_link}")
    await state.clear()
